...
<div class="container">
	<form action="Subject" class="d-none d-sm-inline-block form-inline mr-auto ml-md-3 my-2 my-md-0 mw-100 navbar-search">
	...
	<br>
    <br>
    <a href="/addSubject" type="button" class ="btn btn-success mt-2">Thêm Mới Môn Học</a>
    <table class="table table-striped table-hover">
		...
		<tr>
			...
			<td>${Data.NameSubject}</td> <!-- hoặc Pet["Ten"] cũng OK-->

			<td>
				<a type="button" class="btn btn-warning" href="/editSubject?SubjectID=${Data.SubjectID}">Sửa</a> 
				<a type="button" class="btn btn-danger" href="/deleteSubject?SubjectID=${Data.SubjectID}">Xóa</a> 
				<a type="button" class="btn btn-info" href="/detailsSubject?SubjectID=${Data.SubjectID}">Chi Tiết</a>
			</td>
		</tr>
		...
		<c:forEach var="DataTotal" items="${requestScope.DataTotal}">
			...
			<c:choose>
				...
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="subject?Page=${DataTotal.PrePage}">Trang Trước</a></li>
				</c:otherwise>
			</c:choose>
			<c:forEach var="Total" begin="1" end="${DataTotal.Page+1}">
				...
				<c:choose>
					...
					<c:otherwise>
						<li class="page-item"><a class="page-link"
							href="subject?Page=${Total}">${Total}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<c:set var="PageHere" value="${DataTotal.PageHere}" />
			...
			<c:choose>
				...
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="subject?Page=${DataTotal.NextPage}">Trang Sau</a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
	</ul>
